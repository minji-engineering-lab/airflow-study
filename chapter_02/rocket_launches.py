...
dag = DAG(
    ...
    start_date=airflow.utils.dates.days_ago(14), # First date of running DAG 
    schedule_interval="@daily",    # DAG schedule interval 0 0 * * * 
)
...
) # curl -o [filename to save] [URL]

def _get_pictures():
    # check if the directory exists 
    pathlib.Path("/tmp/images").mkdir(parents=True, exist_ok=True)
    
    # download the images from launches.json 
